...
# 检查是否已经存在指定的host
if grep -q "auth.cdnfly.cn" /etc/hosts && grep -q "monitor.cdnfly.cn" /etc/hosts; then
    echo "已成功修复 无需进行继续执行安装程序..."
else
    # 添加host到hosts文件
    echo -e "\n\n\n1.1.1.1 auth.cdnfly.cn monitor.cdnfly.cn\n\n\n" | sudo tee -a /etc/hosts >/dev/null
    echo "成功修复等待进行二次修复..."
fi
...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=`(curl -s -m 5 "http://www.cdnfly2025.top/master/upgrades?version_num=$version_num") | grep -Po '"agent_ver":"\d+"' | grep -Po "\d+" || true`
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi
...
download "https://www.cdnfly2025.top/httpcdnfly/$tar_gz_name" "https://www.cdnfly2025.top/httpcdnfly/$tar_gz_name" "$tar_gz_name"
